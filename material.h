...
{
public:
  dielectric(double refeaction_index) : refeaction_index(refeaction_index) {}
  bool scatter(const ray &r_in, const hit_record &rec, color &attenuation,
               ray &scattered) const override
  {
    attenuation = color(1.0, 1.0, 1.0);
    double ri = rec.front_face ? (1.0 / refeaction_index) : refeaction_index;
    ...
    vec3 reflected = refract(unit_direction, rec.normal, ri);
    scattered = ray(rec.p, reflected);
    return true;
  }

private:
  double refeaction_index;
